...
builder.Services.AddSingleton<IDnssecCatalog, DnssecCatalog>();
...
try
{
    ...
    await EnsureDnssecSchemaAsync(context);
...
static async Task EnsureDnssecSchemaAsync(DnsContext context)
{
    await context.Database.ExecuteSqlRawAsync(
        """
        CREATE TABLE IF NOT EXISTS "DnssecZones" (
            "Id" integer GENERATED BY DEFAULT AS IDENTITY,
            "Apex" character varying(255) NOT NULL,
            "Enabled" boolean NOT NULL,
            "Algorithm" integer NOT NULL,
            "KskPrivateKeyPem" text NOT NULL,
            "ZskPrivateKeyPem" text NOT NULL,
            "CreatedAt" timestamp with time zone NOT NULL,
            "UpdatedAt" timestamp with time zone NOT NULL,
            CONSTRAINT "PK_DnssecZones" PRIMARY KEY ("Id")
        );
        CREATE UNIQUE INDEX IF NOT EXISTS "IX_DnssecZones_Apex" ON "DnssecZones" ("Apex");
        """
    );
}