...
echo "               On macOS, try TCP first; UDP NodePort to 127.0.0.1 often times out."
echo "DNS (UDP):     dig @127.0.0.1 -p 30053 example.com +short"
echo "               (or dig @${NODE_IP} -p 30053 ...)"
echo "In-cluster:    kubectl run -n sins-rd dig1 --rm -it --restart=Never --image=alpine -- sh -c 'apk add -q bind-tools && dig @sins.sins-rd.svc.cluster.local +dnssec www.example.com A'"
echo ""
